...
function ChckInitLd(){
	...
	if(InitLdd && !MenuUsesFrames)	{clearInterval(LdTmr);Go();}} //Controls selective initial loading;
...
function Go(){
	...
	if(!Ldd){	Crtd=false;Ldd=true;
		...
		try{
			if (parent.menuCookie[menuNumber-1] == 0)
				parent.hideMenu(menuNumber);
		}catch(err){}
...
function HiliteItem(P){
	...
	else{	if(P.Arr[7])P.style.backgroundColor=P.Arr[7];if(P.Arr[9]){P.style.color=P.Arr[9];P.childNodes[0].style.color=P.Arr[9];}//added code to change <a> color
...
function LowItem(P){
	...
	else{	if(P.Arr[6])P.style.backgroundColor=P.Arr[6];if(P.Arr[8]){P.style.color=P.Arr[8];P.childNodes[0].style.color=P.Arr[8];}//added code to change <a> color
...
function OpenMenu(){
//menuOverCounter++;
try{parent.menuClear(menuNumber);}catch(err){}
	if(Ldd&&Crtd){
...
function intervalStarter(thisItem)
{
  ...
  if(intervalNotLocked)												//if we don't already have a countdown running
  {
    ...
    intervalHolder = window.setInterval("finalCountdown()",100);		//create the interval, calling the function finalCountdown every 10 milliseconds
  }
}
...
function finalCountdown()
{
  ...
  if (countdownTimer == 3 && onBool == true)											//after 50 clicks
  {
  ...
  else { countdownTimer++; }										//increment the timer
}
