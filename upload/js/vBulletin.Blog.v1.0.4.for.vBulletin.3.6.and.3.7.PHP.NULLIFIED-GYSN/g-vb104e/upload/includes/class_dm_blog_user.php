...
*
* @package	vBulletin
* @version	$Revision: 25314 $
* @date		$Date: 2007-12-27 18:53:51 -0600 (Thu, 27 Dec 2007) $
*/
class vB_DataManager_Blog_User extends vB_DataManager
{
	...
	*/
	var $validfields = array(
		...
		'options_everyone'    => array(TYPE_UINT,       REQ_NO),
	...
	*/
	var $bitfields = array(
		...
		'options_everyone' => 'bf_misc_vbblogsocnetoptions',
	...
	*/
	function vB_DataManager_BlogUser(&$registry, $errtype = ERRTYPE_STANDARD)
	{
	...
	function pre_save($doquery = true)
	{
		...
		if (!$this->condition)
		{
			...
			$this->set_bitfield('options_everyone', 'canviewmyblog', ($this->registry->bf_misc_vbblogregoptions['viewblog_else'] & $this->registry->options['vbblog_defaultoptions']));
			$this->set_bitfield('options_everyone', 'cancommentmyblog', ($this->registry->bf_misc_vbblogregoptions['commentblog_else'] & $this->registry->options['vbblog_defaultoptions']));
	...
	*/
	function post_delete($doquery = true)
	{
		...
		{
			$blogids = array();
			$blogs = $this->dbobject->query_read("
				SELECT *
			...
			if (sizeof($blogids))
			{
				...
				$attachdata->condition = "blog_attachment.blogid IN ($blogids)";
				$attachdata->delete();
	...
	*/
	function verify_isblogmoderator(&$value)
	{
		if ($value)
		{
			if (!($ismoderator = $this->registry->db->query_first("
				SELECT userid
