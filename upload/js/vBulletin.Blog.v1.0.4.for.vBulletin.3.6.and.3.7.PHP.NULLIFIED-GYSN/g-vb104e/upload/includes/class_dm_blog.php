...
*
* @package	vBulletin
* @version	$Revision: 24003 $
* @date		$Date: 2007-09-28 14:59:34 -0500 (Fri, 28 Sep 2007) $
*/
	...
	function post_save_each_blogtext($doquery = true)
	{
		...
		if ($this->info['posthash'] AND $this->fetch_field('attach') AND $blogid)
		{
			$this->dbobject->query_write("
				...
				WHERE posthash = '" . $this->dbobject->escape_string($this->info['posthash']) . "'
					AND userid = " . intval($this->fetch_field('userid', 'blog_text')) . "
			");
		}
		...
		if (!$this->condition AND $this->fetch_field('state') == 'visible')
		{
			if ($this->info['user'] AND $this->fetch_field('dateline') <= TIMENOW)
			{
				...
				if (!in_coventry($this->fetch_field('userid', 'blog_text'), true))
				{
	...
	*/
	...
	function delete()
	{
		if ($blogid = $this->existing['blogid'])
		{
			...
			if (isset($this->existing['state']) AND $this->existing['state'] == 'visible')
			{
				...
				if (!empty($currentcategories))
				{
					...
					$this->dbobject->query_write("
						...
						SET entrycount = IF(entrycount  - 1 > 0, entrycount - 1, 0)
						WHERE blog_category.blogcategoryid IN (" . implode(',', $currentcategories) . ")
					");
				}
			}
			...
			if ($this->info['hard_delete'])
			{
				...
				$attachdata =& datamanager_init('Attachment_Blog', $this->registry, ERRTYPE_SILENT);
				$attachdata->condition = "blog_attachment.blogid = $blogid";
				$attachdata->delete();
	...
	function build_category_counters()
	{
		...
		{
			if (count($cats = explode(',', $this->registry->vbblog['categorycache']["$userid"]["$categoryid"]['parentlist'])) > 2)
			{
		...
		if ($this->condition)
		{
			...
			$updatecategories = array_merge($deletecategories, $staticcategories_decrease);

			if ($this->existing['state'] == 'visible' AND !empty($updatecategories))
			{
				$this->dbobject->query_write("
					UPDATE " . TABLE_PREFIX . "blog_category AS blog_category
					SET entrycount = IF(entrycount > 0, entrycount - 1, 0)
					WHERE blog_category.blogcategoryid IN (" . implode(',', $updatecategories) . ")
				");
			}
		}

		// increase category counters
		$updatecategories = array_merge($addcategories, $staticcategories_increase);
		if (!empty($updatecategories) AND $this->fetch_field('state') == 'visible' AND !$this->fetch_field('pending'))
		{
			$this->dbobject->query_write("
				UPDATE " . TABLE_PREFIX . "blog_category AS blog_category
			...
			if ($this->fetch_field('state') == 'visible' AND $this->info['blog'] AND $this->info['blog']['state'] == 'visible')
			{
				...
				else
				{
					...
					if ($this->registry->options['enableemail'])
					{
						...
						($hook = vBulletinHook::fetch_hook('blog__post_notification_start')) ? eval($hook) : false;

						$useremails = $this->dbobject->query_read_slave("
							SELECT user.*, blog_subscribeentry.blogsubscribeentryid, blog_moderator.blogmoderatorid, ignored.relationid AS ignoreid, buddy.relationid AS buddyid
						...
						while ($touser = $this->dbobject->fetch_array($useremails))
						{
							...
							if ($userinfo['userid'] != $touser['userid'] AND !($touser['permissions']['vbblog_general_permissions'] & $this->registry->bf_ugp_vbblog_general_permissions['blog_canviewothers']))
							{
								continue;
							}
							else if ($userinfo['userid'] == $touser['userid'] AND !($touser['permissions']['vbblog_general_permissions'] & $this->registry->bf_ugp_vbblog_general_permissions['blog_canviewown']))
							{
								...
								(!$userinfo['buddy_canviewmyblog'] OR !$touser['buddyid'])
									AND
								(!$userinfo['everyone_canviewmyblog'] OR (!$userinfo['buddy_canviewmyblog'] AND $touser['budyid']) OR (!$userinfo['ignore_canviewmyblog'] AND $touser['ignoreid']))
									)
							{
								continue;
							}
...
class vB_DataManager_Blog_Firstpost extends vB_DataManager_Blog
{
	...
	* @param	integer		One of the ERRTYPE_x constants
	*/
	function vB_DataManager_Blogtext_Firstpost(&$registry, $errtype = ERRTYPE_STANDARD)
	{
		parent::vB_DataManager($registry, $errtype);
	...
	*/
	...
	function pre_save($doquery = true)
	{
		...
		else
		{
			...
			if ($this->fetch_field('pending') AND $this->existing['pending'] != 1)
			{
	...
	function post_save_each($doquery = true)
	{
		...
		// Send Email Notification
		if (((!$this->condition AND !$this->fetch_field('pending')) OR $this->info['send_notification']) AND $this->fetch_field('state') == 'visible' AND $this->registry->options['enableemail'])
		{
			$lastposttime = $this->dbobject->query_first("
				SELECT MAX(dateline) AS dateline
			...
			$useremails = $this->dbobject->query_read_slave("
				SELECT user.*, blog_subscribeuser.blogsubscribeuserid, blog_moderator.blogmoderatorid, ignored.relationid AS ignoreid, buddy.relationid AS buddyid
				...
				WHERE blog_subscribeuser.bloguserid = $userid AND
					blog_subscribeuser.type = 'email' AND
					user.usergroupid <> 3 AND
