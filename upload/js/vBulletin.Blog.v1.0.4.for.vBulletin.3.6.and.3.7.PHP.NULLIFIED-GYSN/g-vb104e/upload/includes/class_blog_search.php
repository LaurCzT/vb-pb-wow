...
class vB_Blog_Search
{
	...
	*/
	function execute($search_perms)
	{
		...
		/*echo "<pre>
			SELECT DISTINCT blog.blogid AS id
...
class vB_Blog_SearchGenerator
{
	...
	*/
	var $valid_fields = array(
		...
		'comments_text'  => 'add_comments_text',        // string - fulltext search
		'comments_title' => 'add_comments_title',    // string - fulltext title search
		'textortitle'    => 'add_text_or_title',
	...
	*/
	...
	/**
	* Adds fulltext search criteria, limited to the first post
	*
	* @param	string
	* @param	string
	*
	...
	*/
	function add_text($name, $value)
	{
		...
		$this->joins['left_blog_text'] = trim("
			LEFT JOIN " . TABLE_PREFIX . "blog_text AS blog_text ON (blog_text.blogid = blog.blogid AND blog_text.blogtextid = blog.firstblogtextid)
		");

		$value = $this->registry->db->escape_string($value);

		$this->where['text'] = trim("
			MATCH(blog_text.pagetext) AGAINST ('$value' IN BOOLEAN MODE)
		");
		$this->where['blog_tachy'] = (!empty($this->tachy) ? "blog.userid NOT IN ({$this->tachy})" : '');
	...
	/**
	* Adds full blog text search criteria limited to the first post (title)
	*
	* @param	string
	* @param	string
	*
	...
	* @return	boolean	True on success
	*/
	function add_comments_text($name, $value)
	{
		$value = strval($value);
		if (!$value)
		{
		...
		$this->joins['left_blog_text'] = trim("
			LEFT JOIN " . TABLE_PREFIX . "blog_text AS blog_text ON (blog_text.blogid = blog.blogid)
		");
	...
	* @return	boolean	True on success
	*/
	function add_text_or_title($name, $value)
	{
		$value = strval($value);
		if (!$value)
		{
		...
		$this->where['text'] = trim("
			(MATCH(blog_text.pagetext) AGAINST ('$value' IN BOOLEAN MODE) OR MATCH(blog_text.title) AGAINST ('$value' IN BOOLEAN MODE))
		");
		$this->where['blog_tachy'] = (!empty($this->tachy) ? "blog.userid NOT IN ({$this->tachy})" : '');
		$this->where['blog_text_tachy'] = (!empty($this->tachy) ? "blog_text.userid NOT IN ({$this->tachy})" : '');
