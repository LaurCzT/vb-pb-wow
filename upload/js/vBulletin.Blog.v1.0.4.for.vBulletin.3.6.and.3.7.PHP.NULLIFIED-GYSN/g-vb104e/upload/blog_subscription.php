...
if ($_REQUEST['do'] == 'subscribe')
{
	...
	$url =& $vbulletin->url;
	eval('$content = "' . fetch_template('blog_subscribe_to_item') . '";');
}
...
if ($_REQUEST['do'] == 'userlist')
{
	...
	do
	{
		...
		$getusers = $db->query_read_slave("
			SELECT SQL_CALC_FOUND_ROWS blog_user.entries, blog_user.comments, lastblog, lastblogid, lastblogtitle, blog_user.title, blog_subscribeuser.type, blog_user.ratingnum, blog_user.ratingtotal,
	...
	eval('$content = "' . fetch_template('blog_cp_manage_subscriptions') . '";');
...
if ($_REQUEST['do'] == 'entrylist')
{
	...
	if (!can_moderate_blog())
	{
		...
		$userlist_sql[] = "(options_buddy & " . $vbulletin->bf_misc_vbblogsocnetoptions['canviewmyblog'] . " AND buddy.relationid IS NOT NULL)";
		$userlist_sql[] = "(options_everyone & " . $vbulletin->bf_misc_vbblogsocnetoptions['canviewmyblog'] . " AND (options_buddy & " . $vbulletin->bf_misc_vbblogsocnetoptions['canviewmyblog'] . " OR buddy.relationid IS NULL) AND (options_ignore & " . $vbulletin->bf_misc_vbblogsocnetoptions['canviewmyblog'] . " OR ignored.relationid IS NULL))";
		$wheresql[] = "((" . implode(" OR ", $userlist_sql) . ") AND blog.state = 'visible' AND blog.pending = 0 AND blog.dateline <= " . TIMENOW . ") OR blog.userid = " . $vbulletin->userinfo['userid'];
	...
	do
	{
		...
		$getposts = $db->query_read_slave("
			SELECT SQL_CALC_FOUND_ROWS blog_user.bloguserid, user.userid, blog.lastcomment, blog.lastcommenter, blog.lastblogtextid, options_ignore, options_buddy, options_everyone,
				IF(blog_user.title <> '', blog_user.title, user.username) AS blogtitle, blog_subscribeentry.type, user.username, blog.title, blog.blogid, blog.dateline, blog.ratingtotal, blog.ratingnum,
	...
	if ($db->num_rows($getposts))
	{
		...
		while($post = $db->fetch_array($getposts))
		{
			...
			$show['private'] = false;
			if (can_moderate() AND $post['userid'] != $vbulletin->userinfo['userid'])
			{
				$everyoneelsecanview = $post['options_everyone'] & $vbulletin->bf_misc_vbblogsocnetoptions['canviewmyblog'];
				$buddiescanview = $post['options_buddy'] & $vbulletin->bf_misc_vbblogsocnetoptions['canviewmyblog'];
				if (!$everyoneelsecanview AND (!$post['buddyid'] OR !$buddiescanview))
				{
					$show['private'] = true;
				}
			}

			eval('$blogbits .= "' . fetch_template('blog_cp_manage_subscriptions_entry') . '";');
		}
...
if (empty($navbits))
{
	$navbits = array(
		...
		'blog_usercp.php' . $vbulletin->session->vars['sessionurl_q'] => $vbohrase['blog_control_panel'],
		'' => $vbphrase['blog_entry_subscriptions'],
	);
}
else
{
	$navbits = array_merge(array(
		'blog.php' . $vbulletin->session->vars['sessionurl_q'] => $vbphrase['blogs'],
		'blog_usercp.php' . $vbulletin->session->vars['sessionurl_q'] => $vbphrase['blog_control_panel'],
		'blog_subscription.php?' . $vbulletin->session->vars['sessionurl'] . ($bloginfo['blogid'] ? 'do=entrylist' : 'do=userlist') => $vbphrase['blog_entry_subscriptions'],
	), $navbits);
}
